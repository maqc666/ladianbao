package com.yifu.ladianbao.ui.post_sale.setting
...
class SettingActivity : BaseKActivity() {
    ...
    override fun initView() {
        ...
        tv_set_bank.setOnClickListener { pvCustomOptions!!.show()}
    ...
    private fun initCustomOptionPicker() {
        var selectindex = 6
        if (tv_set_bank.text.isNotEmpty()) {
            for (i in 0 until cardItem.size) {
                if (cardItem[i] == tv_set_bank.text.toString()) {
                    selectindex = i
                    break
                }
            }
